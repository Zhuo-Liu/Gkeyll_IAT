-- Gkyl ------------------------------------------------------------------------
-- Z.Liu 4/27/2021
-- This input file tried to use non-unifrom grids in electron velocity space to expand the simulation box size
-- E2 stands for small electric field, E<E_NL, E << E*
...
local function elc_maxwellian2v_truncated(v, vDrift, vt)
   if -2.5*vtElc < v[1]-vDrift[1] and v[1]-vDrift[1] < 2.5*vtElc then
      return 1/(2*math.pi*vt^2)*math.exp(-((v[1]-vDrift[1])^2+(v[2]-vDrift[2])^2)/(2*vt^2))
   else
...
plasmaApp = Plasma.App {
    ...
    tEnd        = 1200,          -- End time. RLW: I changed this, but didn't change nFrame below.  So we might get more frames.
    nFrame      = 600,             -- Number of output frames.  This is 0.5 frames in unit time --> 3 frames every Langmuir period. 
    ...
    elc = Plasma.Species {
       charge = -1.0, mass = 1.0,
       -- Velocity space grid.
       lower = {-12.0*vtElc,-12.0*vtElc},
       upper = {12.0*vtElc,12.0*vtElc},
       cells = {96,96},
       decompCuts = {1,1},
       -- initial conditions
       init = function (t, xn)
          local x, y, vx, vy = xn[1], xn[2], xn[3], xn[4]
          local fv = elc_maxwellian2v_truncated({vx, vy}, vDriftElc, vtElc)
    ...
    externalField = Plasma.ExternalField {
      ...
      emFunc = function(t, xn)
         local extE_x, extE_y, extE_z = -0.001, 0., 0.
