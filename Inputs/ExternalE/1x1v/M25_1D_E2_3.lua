...
local function maxwellian1v(v, vDrift, vt)
    if v-vDrift > 4.0*vtElc then	
       return 0.00001*1/(2*math.pi*vt^2)*math.exp(-((v-vDrift)^2)/(2*vt^2))
    elseif v-vDrift <- 4.0*vtElc then
       return 0.00001*1/(2*math.pi*vt^2)*math.exp(-((v-vDrift)^2)/(2*vt^2))
    else
       return 1/(2*math.pi*vt^2)*math.exp(-((v-vDrift)^2)/(2*vt^2))
    end
end
...
plasmaApp = Plasma.App {
    ...
    ion = Plasma.Species {
       ...
       init = function (t, xn)
          ...
          local fv = maxwellian2v(vx, vDriftIon, vtIon)
          local dfv = sponEmissionSource({x,vx},t, lx, nx, pOrder)[1]*maxwellian2v(vx, vDriftIon, vtIon)
          return fv + perturbation*dfv    -- *(1.0 + perturbation*DrandKick({x,y},lx,nx,pOrder) ) RLW: I removed the noise
       end,
       source = Plasma.Source{
	   profile = function (t,xn)
         local x, vx = xn[1], xn[2]
         local dfv = sponEmissionSource({x,vx},t, lx, nx, pOrder)[1]*maxwellian2v(vx,accion*t, vtIon) 
         return noise*dfv
       end,        
       },
