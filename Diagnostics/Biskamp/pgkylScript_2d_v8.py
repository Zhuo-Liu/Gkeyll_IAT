...
outDir     = './gkyl_plots_v8/'               #.Output directory where figures are saved.
fourier_transform = True
auto_loading      = False
#creating the directory for plots if it does not exist yet
if not os.path.exists('gkyl_plots_v8'):
    os.makedirs('gkyl_plots_v8')
...
#    eField_fluct_z = np.zeros(nz, ny, nFrames) attempt!
...
#        e_raw = np.squeeze(pgu.getRawData(fName_field))
       ...
       # iond = np.squeeze(pgu.getInterpData(fName,polyOrder,basisType))
        ...
        ## Get dimensions of velocity grid for both species
        ...
        #rlw: get z coordinate of center of box (it is 48)
