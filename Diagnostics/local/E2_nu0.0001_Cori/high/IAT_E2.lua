-- Gkyl ------------------------------------------------------------------------
-- Z.Liu 7/20/2021
-- This input file is ultimate version for old gkeyll, E2
-- 1. Remove electron ion collisions, because that is not physical, and comsumed lots of resourses
-- 2. Considering reducing the electron electron collisions (higher collisions result it more severe boundary problem)
-- 3. Increase electron velocity range to {-6 vtElc, 14 vt Elc}
-- 4. Increase the upper boundary of ion velocity space to 32 vtIon
-- 4. Remove unnecessary Int Moment diagnostics
-- 5. Apply noise on ions dynamically
-- 6. Remove the use of truncated ions, because that is bad
...
local function maxwellian2v_ion(v, vDrift, vt)
    if v[1]-vDrift[1] > 8.0*vtIon then	
       return 0.00001*1/(2*math.pi*vt^2)*math.exp(-((v[1]-vDrift[1])^2+(v[2]-vDrift[2])^2)/(2*vt^2))
    elseif v[1]-vDrift[1] < -8.0*vtIon then
       return 0.00001*1/(2*math.pi*vt^2)*math.exp(-((v[1]-vDrift[1])^2+(v[2]-vDrift[2])^2)/(2*vt^2))
    else
       return 1/(2*math.pi*vt^2)*math.exp(-((v[1]-vDrift[1])^2+(v[2]-vDrift[2])^2)/(2*vt^2))
    end
...
plasmaApp = Plasma.App {
    ...
    elc = Plasma.Species {
       ...
       diagnosticMoments           = {},
       diagnosticIntegratedMoments = {"intM1i", "intM2Thermal"},
    ...
    ion = Plasma.Species {
       ...
       -- Velocity space grid.
       lower = {-24.0*vtIon,-16.0*vtIon},
       upper = {36.0*vtIon,16.0*vtIon},
       cells = {60,32},
       decompCuts = {1,1},
       -- Initial conditions.
       init = function (t, xn)
