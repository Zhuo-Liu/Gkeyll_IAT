...
fileName   = 'M25_E2_3'    #.Root name of files to process.
dataDir = '../'
outDir  = './'
...
def get_field_data(nFr):
    ...
    phi_full = pgu.getInterpData(fName,polyOrder,basisType)
    print(np.array(phi_full).shape)
...
def plot_spec(nFr,frameWindow=None,singleShot=True):
    if singleShot == True:
        ...
        pos0 = axs[0].contourf(K_z, K_y, np.log(field_E_fluct_square_K))
...
def save_velocity_grid():
    ...
    z_elc = np.array(x_elc[0])
    y_elc = np.array(x_elc[1])

    np.savez('./dist_function/elc_velocities',velocitiesz_elc, velocitiesy_elc)
    np.savez('./dist_function/ion_velocities',velocitiesz_ion, velocitiesy_ion)
    np.savez('./dist_function/sapce',z_elc,y_elc)
...
def save_distribution_function(nFr):
    ...
    elcd_space = np.average(elcd,axis=(2,3))
    iond_space = np.average(iond,axis=(2,3))

    elcd_phase = np.average(elcd,axis=(1,3))
    iond_phase = np.average(iond,axis=(1,3))
    ...
    save_name_elc_space = './dist_function/' + rf'{time}' + '_elc_space.txt'
    save_name_ion_space = './dist_function/' + rf'{time}' + '_ion_space.txt'
    save_name_elc_phase = './dist_function/' + rf'{time}' + '_elc_phase.txt'
    save_name_ion_phase = './dist_function/' + rf'{time}' + '_ion_phase.txt'
    ...
    np.savetxt(save_name_elc_space, elcd_space)
    np.savetxt(save_name_ion_space, iond_space)

    np.savetxt(save_name_elc_phase, elcd_phase)
    np.savetxt(save_name_ion_phase, iond_phase)
...
iTw_frequency = [int((nFrames-1)*0.75), int((nFrames-1)*0.9)]
...
#save_field(iTw_dis)
#save_velocity_grid()
#save_distribution_function(75)
#save_distribution_function(100)
#save_distribution_function(120)
#save_distribution_function(150)
#save_distribution_function(175)
#save_distribution_function(200)
#save_distribution_function(240)
#save_distribution_function(360)
#save_distribution_function(480)
save_distribution_function_window(iTw_dis)
